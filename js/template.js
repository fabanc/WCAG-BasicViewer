...
define([
    ...
    return declare([Evented], {
        ...
        constructor: function(templateConfig) {
            ...
            this.templateConfig = lang.mixin(
        ...
        _init: function() {
            ...
            this.customUrlConfig = this._getUrlParamValues(
                this.templateConfig.urlItems
            );
        ...
        _completeApplication: function() {
            ...
            if (
                this.config.appid &&
                this.config.application_extent &&
      ...
      */
            lang.mixin(
                ...
                this.orgConfig,
                this.appConfig,
                this.groupInfoConfig,
                this.groupItemConfig,
                this.displayItemConfig,
                this.customUrlConfig,
                this.urlConfig
            );
        },
        _createPortal: function() {
            ...
            if (
                this.templateConfig.queryForGroupInfo ||
                this.templateConfig.queryForGroupItems
            ) {
            ...
            if (urlObject && urlObject.query && items && items.length) {
                for (i = 0; i < items.length; i++) {
                    if (urlObject.query[items[i]]) {
                        ...
                        obj[items[i]] =
                            item === "true" || (item == "false" ? false : item);
                    }
                }
            }
            return obj;
        ...
        _checkSignIn: function() {
            ...
            signedIn = IdentityManager.checkSignInStatus(
                this.config.sharinghost + "/sharing"
            );
        ...
        _getLocalization: function() {
            ...
            if (this.templateConfig.queryForLocale) {
                require(["dojo/i18n!application/nls/resources"], lang.hitch(
                    this,
                    function(appBundle) {
                        ...
                        if (this.i18nConfig.i18n.direction === "rtl") {
                            ...
                            rtlClasses =
                                " esriRTL dj_rtl dijitRtl " +
                                classes.replace(/ /g, "-rtl ");
        ...
        queryGroupItems: function(options) {
            ...
            if (this.templateConfig.queryForGroupItems) {
                if (this.config.group) {
                    ...
                    params = lang.mixin(
                        defaultParams,
                        this.templateConfig.groupParams,
                        options
                    );
                    // place group ID
                    if (params.q) {
        ...
        _queryDisplayItem: function() {
            ...
            if (this.templateConfig.queryForWebmap) {
                ...
                arcgisUtils.getItem(this.config.webmap).then(
                    lang.hitch(this, function(itemInfo) {
                ...
                arcgisUtils.getItem(this.config.appid).then(
                    lang.hitch(this, function(response) {
                        ...
                        if (response.item && response.item.extent) {
                            this.appConfig.application_extent =
                                response.item.extent;
                        }
                        deferred.resolve(response);
                    }),
                    function(error) {
                        if (!error) {
                            error = new Error(
                                "Error retrieving application configuration."
                            );
                        }
                        deferred.reject(error);
                    }
            ...
            if (this.templateConfig.queryForOrg) {
                ...
                esriRequest({
                    ...
                    lang.hitch(this, function(response) {
                        ...
                        var q = this._parseQuery(
                            response.basemapGalleryGroupQuery
                        );
                        ...
                        if (
                            response.user &&
                            esriLang.isDefined(response.user.roleId)
                        ) {
                            if (response.user.privileges) {
                                this.orgConfig.userPrivileges =
                                    response.user.privileges;
                            }
                        }
                        deferred.resolve(response);
                    }),
        ...
        _parseQuery: function(queryString) {
            ...
            while (result) {
                ...
                fields[fieldName] = queryString
                    .substring(
                        fieldIndex,
                        result ? result.index : queryString.length
                    )
                    .replace(/^\s+|\s+$/g, "")
